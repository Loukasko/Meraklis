            ...
            <div class="col-sm-6">
                <div class="card text-center">
                    ...
                    <div class="card-body">
                        ...
                        {{--You are logged in as Lawbreaker!--}}

                        <div class="container">
                            <div class="row">
                                <div class="align-middle">
                                    {{--@if(($_REQUEST->user('lawbreaker')->status)==0)--}}
                                    ...
                                    <form action="POST" id="getLocation">
                                        <button type="button" class="btn btn-success" id="map-btn">Επιλογή τοποθεσίας</button>
                                    </form>
                                    {{--<form method="GET" action="{{ route('lawbreaker.getLocation') }}" aria-label="{{ __('Επιλογή τοποθεσίας') }}">--}}


                                    {{--</form>--}}
                                    {{--<iframe src="{{ url('lawbreaker/home/show-map') }}"></iframe>--}}
                                    {{--<link rel="import" href="{{ url('lawbreaker/home/show-map') }}">--}}
                ...
                geocoder.geocode({'latLng': marker.getPosition()}, function (results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        ...
                        $('#getLocation').click(function(event){
                            ...
                            var lat = point.lat();
                            var lng = point.lng();
                            $.post('get-location',{'address':address,'lat': lat,'lng':lng,'_token':$('input[name=_token]').val()},function(data){
                                console.log(data);
                            });
                        })
                    }
            ...
            autocomplete.addListener('place_changed', function() {
                ...
                $('#getLocation').click(function(event){
                    ...
                    $.post('get-location',{'address':address,'lat': lat,'lng':lng,'_token':$('input[name=_token]').val()},function(data){
                     ...
                     window.location = "lawbreaker/home";
                });
